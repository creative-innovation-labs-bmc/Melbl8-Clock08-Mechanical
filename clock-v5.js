...
function fitStage() {
  ...
  if (debugMode) {
    debug.hidden = false;
    debug.textContent = `${window.innerWidth}x${window.innerHeight} | scale ${scale.toFixed(4)} | V5 fast wind`;
  }
}
...
function windAngle(cell, seconds) {
  const drift = seconds * 2.5;
  const broadWave = 22 * Math.sin(seconds * 0.44 - cell._windX * 0.34);
  const crossWave = 9 * Math.sin(seconds * 0.62 + cell._windY * 0.72 + cell._windPhase);
  const longGust = 7 * Math.sin(seconds * 0.15 + cell._windX * 0.12);
...
function updateWind(force = false) {
  if (document.hidden) return;
  for (const cell of windCells) {
    if (!cell._isActive) moveInactiveCellWithWind(cell, force);
  }
}

function buildClockPage() {
  const digitEls = [];
  ['hours', 'minutes', 'seconds'].forEach((id, digitOrdinalBlock) => {
    const group = document.getElementById(id);
    for (let i = 0; i < 2; i++) {
      const digit = createDigit(digitOrdinalBlock * 2 + i);
  ...
  function scheduleWind() {
    updateWind(false);
    window.setTimeout(scheduleWind, WIND_TICK_MS);
  }
